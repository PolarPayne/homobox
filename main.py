...
def chat():
    if request.method == 'POST' and session.get('logged_in', False) and request.form['type'] == 'shout':
        update_db('insert into shouts(user_id, shout, post_time) values (?, ?, datetime(\'now\', \'localtime\'))',
                  [session['user_id'], request.form['content']])
        return redirect(url_for('chat'))

    if session.get('logged_in', False):
        shouts = []
        for shout in query_db('select name, shout, strftime(\'%Y-%m-%d %H:%M\', post_time) from users'
                              ' natural join shouts order by shout_id desc limit 15'):
...
def chat_log(amount=50):
    if session.get('logged_in', False):
        shouts = []
        for shout in query_db('select name, shout, strftime(\'%Y-%m-%d %H:%M\', post_time) from users'
                              ' natural join shouts limit ?', [amount]):
            shouts.insert(0, (shout[0], shout[1], shout[2]))
    ...
    if request.method == 'POST':
        ...
        if session.get('logged_in', False):
            if request.form['type'] == 'change_password':
                if not change_password(session.get('name', ''),
                                       session.get('user_id', ''),
                                       request.form['old_password'],
                                       request.form['new_password'],
                                       request.form['new_password_again']):
...
def new_user(name, password, admin=False):
    if len(name) < 3 or len(password) < 8 or \
                    query_db('select count(*) from users where name = ?', [name], one=True)[0] > 0:
        return False
    update_db('insert into users(name, password, admin) values (?, ?, ?)',
              [name, bcrypt.generate_password_hash(name + password), admin])
    return True
...
def change_password(name, user_id, old_password, new_password, new_password_again):
    if new_password == new_password_again and check_password(user_id, old_password):
        update_db('update users set password = ? where user_id = ?',
                  [generate_password_hash(name, new_password), user_id])
        return True
    return False
